...
try {
    ...
    $checkCols = $mysqli->query("SHOW COLUMNS FROM users LIKE 'profesi'");
    if ($checkCols->num_rows == 0) {
        $mysqli->query("ALTER TABLE users 
            ADD COLUMN nama_panggilan VARCHAR(50) DEFAULT NULL,
            ADD COLUMN profesi VARCHAR(100) DEFAULT NULL,
            ADD COLUMN role ENUM('user', 'admin', 'superadmin') DEFAULT 'user'
        ");
