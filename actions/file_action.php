...
if (
    ...
    $stype = $_POST['item_type'] ?? 'file'; // 'folder' | 'file' | 'link'
    $table = ($stype === 'folder') ? 'folders' : 'files';
    $col   = ($stype === 'folder') ? 'id' : 'id';
 
    // Ambil nilai is_starred saat ini
    $stmt = $mysqli->prepare("SELECT is_starred FROM `{$table}` WHERE id=?");
    ...
    $new_val = $row['is_starred'] ? 0 : 1;
 
    // Update — hanya pemilik atau admin yang bisa bintangi
    if (isAdmin()) {
        $stmt = $mysqli->prepare("UPDATE `{$table}` SET is_starred=? WHERE id=?");
        $stmt->bind_param('ii', $new_val, $sid);
    } else {
        $stmt = $mysqli->prepare("UPDATE `{$table}` SET is_starred=? WHERE id=? AND owner_username=?");
        $stmt->bind_param('iis', $new_val, $sid, $username);
    }
    $stmt->execute();
    $stmt->close();
 
    header('Content-Type: application/json');
    echo json_encode(['ok' => true, 'starred' => (bool)$new_val]);
    exit;
}
